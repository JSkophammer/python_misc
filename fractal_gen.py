...
def func_comp(length, f, dim):
    return lambda x, y, z: ext_draw(length, lambda a, b, c: f, dim)
...
def draw_repeated(length, dim):
    orient_turtle()
    ext_draw(length,
             lambda x, y, z: ext_draw(length,
                                      lambda a, b, c: ext_draw(length, draw_shape, dim - 2),
                                      dim - 1),
             dim)
    window = turtle.Screen()
    window.exitonclick()
...
if __name__ == "__main__":
    orient_turtle(1)
    eval(gen_fract_exp(3, 15, 5))
    main_window.exitonclick()
